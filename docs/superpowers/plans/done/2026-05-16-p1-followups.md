...
> **Status:** ✅ Shipped — merged into `develop` via `feature/p1-followups` on 2026-05-17 (merge commit `91a10eb`, PR #44). All four review-raised follow-ups are live: `StartDate` made non-nullable on `CreateTrackedTaskDto`/`UpdateTrackedTaskDto` (with a `NonDefaultDateTimeOffsetAttribute` presence guard added during review), `UserController.GetMe` aggregates project/task counts and total tracked time via new `IRepository<T>.CountAsync`/`SumAsync`, `theme-bootstrap.js` notifies Blazor on OS-color-scheme changes through `ThemeService.RegisterOsChangeListenerAsync` + `[JSInvokable] NotifyResolvedThemeChangedAsync`, and a new `UserProfileService` caches `/User/me` so a session makes one read instead of 3-4. Task 10 (end-to-end smoke) is a manual hand-off; running it post-merge found `GET /User/me` 500ing against real SQL Server — `SumAsync` cannot translate `TimeSpan.Ticks` into a SQL `SUM`, and PR #44's EF-InMemory unit tests never exercised SQL translation. PR #45 (`feature/p1-followups-tests`) fixes `SumAsync` (materialize the projected column, sum in memory), adds a SQLite-backed regression test, and adds the missing `UserProfileService`/`ThemeService` client coverage.
