...
> **Status:** ✅ Shipped — merged to `develop` via PR #46 (2026-06-03), released as v2.2 via PR #47 (2026-06-12). Tasks 1–15 completed (checkboxes below were not ticked during execution). Of the v2.2 tech-debt sweep, only the DB indexes + filtered unique `Project(UserId, Name)` constraint shipped; the remaining five items (CI workflow enable, Constants split, request logging, API versioning, validation integration tests) carry over to v2.3.
